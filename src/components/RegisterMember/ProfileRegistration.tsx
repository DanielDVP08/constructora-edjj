...
export default function ProfileRegistration({
  ...
  const handleSelectChange = (name: string, value: string) => {
    if (name === "department") {
      setFormData((prev) => ({
        ...prev,
        [name]: Localidades[Number.parseInt(value)].departamento,
      }));
      // setProvinces(["Province 1", "Province 2", "Province 3"]);
      setProvinces(Localidades[Number.parseInt(value)].provincias);
      setFormData((prev) => ({ ...prev, province: "" }));
      // setDistricts([]);
    } else if (name === "province") {
      const index = departaments.indexOf(formData.department);
      setFormData((prev) => ({
        ...prev,
        [name]: Localidades[index].provincias[Number.parseInt(value)],
      }));
    } else {
      setFormData((prev) => ({
        ...prev,
  ...
  const handleSubmit = async (e: React.SyntheticEvent) => {
    ...
    form.set("location", `${formData.department},${formData.province}`);
  ...
  return (
    <div className="min-h-screen bg-gray-100 flex flex-col">
      ...
      <main className="flex-grow flex items-center justify-center py-12 px-4 sm:px-6 lg:px-8">
        <Card className="w-full max-w-4xl bg-amber-200">
                  ...
                  <div className="space-y-4">
                    <div>
                      ...
                      <Select
                        ...
                        placeholder="Selecciona tu ultimo nivel de Educacion"
                      >
                        <SelectItem key="Secundaria Completa">
                          Secundaria Completa
                        </SelectItem>
                        <SelectItem key="Bachiller">Bachiller</SelectItem>
                        <SelectItem key="Colegiatura">Colegiatura</SelectItem>
